...
struct ImGuiNeoSequencerInternalData
{
    ...
    ImVec4 CurrentFrameColor;  // Color of current frame, we have to save it because we render on EndNeoSequencer, but process
                               // at BeginneoSequencer
...
static void addKeyframeToDeleteData(int32_t value, ImGuiNeoSequencerInternalData& context, const ImGuiID timelineId)
{
    ...
    for (auto&& val : context.SelectionData)
    {
        ...
        {
            foundTimeline = true;
            if (!val.KeyframesToDelete.contains(value)) val.KeyframesToDelete.push_back(value);
            break;
        }
    }
...
{
    // TODO(matej.vrba): This is kinda slow, it works for smaller data sample, but for bigger sample it should be changed to
    // hashset
                                 ...
                                 FrameIndexType* end)
{
    ...
    if (endFrameVal <= startFrameVal) endFrameVal = (int32_t)*end;
    ...
    *end = endFrameVal;
    ...
    if (hovered)
    {
        ...
        if (*start + context.OffsetFrame + newZoomWidth > *end) context.OffsetFrame = ImMax(0U, totalFrames - viewWidth);
    }
    ...
    else
    {
        switch (context.StateOfSelection)
        {
            ...
            case SelectionState::Dragging:
            {
                ...
                for (auto&& t : context.SelectionData) t.KeyframesToDelete.resize(0);
                break;
            }
        }
    }
...
static void renderSelection(ImGuiNeoSequencerInternalData& context)
{
    ...
    if (sel.GetArea() < 32.0f) return;

    // Inner
    drawList->AddRectFilled(context.SelectionMouseStart,
                            currentMousePosition,
                            ColorConvertFloat4ToU32(style.Colors[ImGuiNeoSequencerCol_Selection]));

    // border
    drawList->AddRect(context.SelectionMouseStart,
                      currentMousePosition,
...
static bool groupBehaviour(const ImGuiID id, bool* open, const ImVec2 labelSize)
{
    ...
    if (addArrowRes)
    {
        if (IsItemClicked() && closable) (*open) = !(*open);
    }
...
bool BeginNeoSequencer(const char* idin,
                       ...
                       FrameIndexType* startFrame,
                       FrameIndexType* endFrame,
                       const ImVec2& size,
                       ImGuiNeoSequencerFlags flags)
{
    ...
    if (realSize.x <= 0.0f) realSize.x = ImMax(4.0f, area.x);
    if (realSize.y <= 0.0f) realSize.y = ImMax(4.0f, context.FilledHeight);
    ...
    if (context.TopBarSize.y <= 0.0f) context.TopBarSize.y = CalcTextSize("100").y + imStyle.FramePadding.y * 2.0f;
    ...
    RenderNeoSequencerBackground(GetStyleNeoSequencerColorVec4(ImGuiNeoSequencerCol_Bg),
    ...
    RenderNeoSequencerTopBarOverlay(context.Zoom,
                                    context.ValuesWidth,
                                    context.StartFrame,
                                    context.EndFrame,
                                    context.OffsetFrame,
                                    ...
                                    context.TopBarSize,
                                    drawList,
                                    style.TopBarShowFrameLines,
                                    style.TopBarShowFrameTexts,
                                    style.MaxSizePerTick);

    if (showZoom)
        processAndRenderZoom(context,
                             context.TopLeftCursor,
                             flags & ImGuiNeoSequencerFlags_AllowLengthChanging,
                             startFrame,
                             endFrame);

    if (context.Size.y < context.FilledHeight) context.Size.y = context.FilledHeight;
    ...
    if (showZoom) context.StartValuesCursor = context.StartValuesCursor + ImVec2{0, calculateZoomBarHeight()};
    ...
    drawList->PushClipRect(
        clipMin,
        clipMin + backgroundSize - ImVec2(0, context.TopBarSize.y) - ImVec2{0, GetFontSize() * style.ZoomHeightScale},
        true);
...
void EndNeoSequencer()
{
    ...
    if (context.SelectionEnabled) processSelection(context);

    context.LastSelectedTimeline = context.SelectedTimeline;
    context.IsSelectionRightClicked = false;

    if (context.SelectionEnabled) renderSelection(context);
...
bool BeginNeoTimeline(const char* label, std::vector<int32_t>& keyframes, bool* open, ImGuiNeoTimelineFlags flags)
{
    ...
    for (uint32_t i = 0; i < keyframes.size(); i++) c_keyframes[i] = &keyframes[i];
...
bool BeginNeoTimelineEx(const char* label, bool* open, ImGuiNeoTimelineFlags flags)
{
    ...
    if (addRes)
    {
        ...
        RenderNeoTimelineLabel(
            label,
            context.ValuesCursor + imStyle.FramePadding + ImVec2{(float)currentTimelineDepth * style.DepthItemSpacing, 0},
            labelSize,
            color,
            isGroup,
            isGroup && (*open));
    ...
    if (result) context.TimelineStack.push_back(id);
...
bool BeginNeoTimeline(const char* label,
                      FrameIndexType** keyframes,
                      uint32_t keyframeCount,
                      bool* open,
                      ImGuiNeoTimelineFlags flags)
{
    if (!BeginNeoTimelineEx(label, open, flags)) return false;
...
uint32_t GetNeoKeyframeSelectionSize()
{
    ...
    if (!context.DeleteEnabled) return 0;
    ...
    {
        if (deleteSelection.TimelineID == timelineId) return deleteSelection.KeyframesToDelete.size();
    }
...
void GetNeoKeyframeSelection(FrameIndexType* selection)
{
    ...
    if (!context.DeleteEnabled) return;
